...
public class AcrobaticsManager extends SkillManager {
    ...
    public double roll(double damage) {
        ...
        if (!isFatal(modifiedDamage) && isSuccessfulRoll(isGraceful)) {
            ...
            player.sendMessage(isGraceful ? LocaleLoader.getString("Acrobatics.Ability.Proc") : LocaleLoader.getString("Acrobatics.Roll.Text"));
    ...
    private boolean canDodge(Entity damager, double modifiedDamage) {
        return (Permissions.dodge(getPlayer()) && !(damager instanceof LightningStrike && Acrobatics.dodgeLightningDisabled) && CombatUtils.shouldProcessSkill(damager, skill) && SkillUtils.activationSuccessful(getSkillLevel(), getActivationChance(), Acrobatics.dodgeMaxChance, Acrobatics.dodgeMaxBonusLevel) && !isFatal(modifiedDamage));
    }
}
